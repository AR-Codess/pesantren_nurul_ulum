...
Route::middleware(['auth:web', 'role:user|web'])->group(function () {
    Route::post('/pembayaran/midtrans/token', [PembayaranController::class, 'createMidtransToken'])->name('pembayaran.midtrans.token');
});
// Midtrans notification callback (no auth)
Route::post('/midtrans/callback', [PembayaranController::class, 'midtransCallback'])->name('midtrans.callback');
